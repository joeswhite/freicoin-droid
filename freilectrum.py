...
#
# Updates copyright 2014 joe@freicoin.us
#
...
def print_help(parser):
    parser.print_help()
    print_msg("Type 'freilectrum help <command>' to see the help for a specific command")
    print_msg("Type 'freilectrum --help' to see the list of options")
...
if __name__ == '__main__':
    ...
    if cmd == 'daemon':
        ...
        if arg not in ['start', 'stop', 'status']:
            print_msg("syntax: freilectrum daemon <start|status|stop>")
            sys.exit(1)
    ...
    if cmd.name not in ['create', 'restore'] and cmd.requires_wallet and not storage.file_exists:
        ...
        print_msg("Type 'freilectrum create' to create a new wallet, or provide a path to a wallet with the -w option")
        sys.exit(0)
