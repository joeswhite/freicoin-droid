...
class Abstract_Wallet(object):
    ...
    def __init__(self, storage):
        ...
        self.fee                   = int(storage.get('fee_per_kb', RECOMMENDED_FEE))
...
#    wallet_type = 'old'
	...
	#import old init_seeds for android version
    # def init_seed(self, seed):
        ...
        # self.seed = old_mnemonic.mn_decode(words)
    ...
    def init_seed(self, seed):
        ...
        words = seed.split()
        try:
            Mnemonic(lang).mnemonic_decode(words)
        except Exception:
            raise

        self.seed = Mnemonic(lang).mnemonic_decode(words)
    ...
    def create_master_keys(self, version, password):
	    #this is a null variable for use later 
	    #this is the version sig (1of1 2of2 2of3)
        sigversion = version
        seed = self.get_seed(password)
