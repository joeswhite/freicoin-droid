...
def pay_to(recipient, amount, fee, label):
    ...
    droid.dialogCreateSpinnerProgress("FreiLectrum", "signing transaction...")
    droid.dialogShow()
...
#        modal_dialog('error', str(e.message + '-Did you too many coins?'))
        modal_dialog('error', str(e.message) + 'Do you have enough coins? Are your coins confirmed?')
        droid.dialogDismiss()
        return
...
class ElectrumGui:
    ...
    def restore_wallet(self):
        ...
        droid.dialogCreateSpinnerProgress("FreiLectrum", msg)
        droid.dialogShow()
